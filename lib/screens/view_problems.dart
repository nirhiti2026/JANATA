...
class ViewProblems extends StatefulWidget {
  const ViewProblems({super.key});

  @override
  State<ViewProblems> createState() => _ViewProblemsState();
}

class _ViewProblemsState extends State<ViewProblems> {
  ...
  List<Problem> _applyFilter(List<Problem> all) {
    ...
    switch (_searchBy) {
      case 'Date':
        return all.where((p) => p.createdAt.toDate().toString().toLowerCase().contains(ql)).toList();
      case 'Province':
        return all.where((p) => p.province.toLowerCase().contains(ql)).toList();
      case 'Ward No':
        return all.where((p) => p.wardNo.toLowerCase().contains(ql)).toList();
      ...
      case 'Name':
        return all.where((p) => p.title.toLowerCase().contains(ql)).toList();
      case 'Problem Type':
        return all.where((p) => p.problemType.toLowerCase().contains(ql)).toList();
      default:
        return all;
    }
  }
  ...
  Widget build(BuildContext context) {
    final fs = Provider.of<FirestoreService>(context, listen: false);
    return Scaffold(
      appBar: AppBar(title: const Text('View All Problems')),
      body: Column(
        children: [
          Padding(
                  ...
                  return Center(
                    child: Column(
                      ...
                      children: const [
                        ...
                        SizedBox(height: 8),
                        Text('Error loading problems', style: TextStyle(color: Colors.grey)),
                      ],
                    ),
                  );
                }
